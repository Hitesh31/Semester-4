...
{
    int n;
    in.open("input.txt"); // Reads from input.txt
    in>>n;
    ...
    for(int i=0;i<n;i++)
    {
        ...
        <<task[i].deadline<<"; period:"<<task[i].period<<" joined the system at t = 0\n";
    }
    ...
    while(1)
    {
        ...
        sort(ready_queue.begin(),ready_queue.end(),period_sort()); //sorts the queue according to period
        ...
        if(current.on_cpu==false)
        {
            ...
            else
            {
                ...
                outf<<"CPU is idle till "<<t-1<<".\n";
                continue;
            }
        }

        
        if(!ready_queue.empty() && ready_queue[0]->period < task[current.index].period) 
        { //preemption
            // if the top of queue has period less than current process , it is preempted and runnign process is pushed to  queue
    ...
    ofstream av;
    av.open("average.txt");
    av<<"RMS : Average wait time : "<<average_wait<<"; Average turnaround time : "<<td_time<<"\n";
    av.close();
    outf.close();
    outf.open("RM-Stats.txt");
